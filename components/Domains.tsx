    ...
    <section id="domains" className="pt-40 pb-24 bg-transparent page-break relative">
      <div className="container mx-auto px-6">
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <h3 className="text-4xl font-extrabold text-slate-900 mb-6">Specialized Tracks for Every Career Path</h3>
          ...
          {DOMAINS.map((domain) => (
            <div
              ...
              className="
                ...
                border border-slate-200 shadow-lg shadow-slate-200/50 flex flex-col 
            ...
            >
              ...
              <p className="text-slate-600 text-sm mb-6 flex-grow">{domain.description}</p>

              <div className="space-y-3">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Key Topics</p>
                ...
                {domain.topics.length > 4 && (
                  <div className="text-xs text-slate-400 pl-6">+ {domain.topics.length - 4} more</div>
                )}
              </div>
      ...
      {activeDomain && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
          ...
          {/* Content */}
          <div className="relative bg-white border border-slate-200 w-full max-w-5xl max-h-[90vh] overflow-y-auto rounded-3xl shadow-2xl flex flex-col">

            {/* Modal Header */}
            <div className="sticky top-0 bg-white/95 backdrop-blur z-10 p-8 border-b border-slate-200 flex justify-between items-start">
              <div>
                <h3 className="text-3xl font-bold text-slate-900 mb-2">{activeDomain.name} Projects</h3>
                <p className="text-slate-600">{activeDomain.description}</p>
              </div>
              <button
                onClick={() => setActiveDomainId(null)}
            ...
            <div className="p-8 grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {activeDomain.projects && activeDomain.projects.length > 0 ? (
                activeDomain.projects.map((project, idx) => (
                  <div key={idx} className="bg-slate-50 border border-slate-200 rounded-2xl overflow-hidden hover:border-blue-500/40 transition-all group/card flex flex-col">
                    ...
                    <div className="p-6 flex-grow flex flex-col">
                      <h5 className="text-lg font-bold text-slate-900 mb-2">{project.title}</h5>
                      ...
                      <div className="flex flex-wrap gap-2 mb-6">
                        {project.tags.map((tag, tIdx) => (
                          <span key={tIdx} className="px-2 py-1 bg-white text-slate-600 text-[10px] uppercase font-bold tracking-wider rounded border border-slate-200 shadow-xs">
                            {tag}
                          </span>
                        ))}
                      </div>
            ...
            {/* Modal Footer */}
            <div className="p-6 bg-slate-50 border-t border-slate-200 flex flex-col sm:flex-row justify-center gap-4">
              <Link
                to="/internship"
                onClick={() => setActiveDomainId(null)}
              ...
              <Link
                to="/contact"
                onClick={() => setActiveDomainId(null)}
                className="px-8 py-3 bg-slate-200 hover:bg-slate-300 text-slate-800 font-bold rounded-xl transition-colors text-center"
              >
